...
class WasmInterface {
  ...
  getPitch() {
    return wasm_instance.exports.getPitch(scene.ptr, scene.get_pitch_fn_ptr);
  }
...
const env = {
  ...
  setInterval(fn_ptr, delay, count) {
    ...
    if (count > 0)
      setTimeout(() => clearInterval(interval_handle), count * delay);
